...
async def generate_rag_script(req: RagRequest):
    ...
    prompt = PromptTemplate(
        ...
        template=(
            "당신은 뉴스 기사를 기반으로 정확하고 풍부한 대화를 생성하는 전문가입니다.\n\n"
            
            "## 주어진 정보\n"
            "**뉴스 기사:**\n{content}\n\n"
            "**기존 대화:**\n{originalScript}\n\n"
            "**추가 참고 자료 (나무위키, arXiv 논문 등):**\n{context}\n\n"
            
            "## 임무\n"
            "위 정보들을 종합적으로 활용하여 두 캐릭터 간의 새로운 QnA 대사를 생성해주세요.\n\n"
            ...
            "## 핵심 지침 (매우 중요!)\n"
            "### 1. 정확성 및 환각 방지\n"
            "- 뉴스 기사와 참고 자료에서 확인할 수 있는 정보만 사용하세요\n"
            "- 확실하지 않은 정보는 절대 추가하지 마세요\n"
            "- 숫자, 날짜, 고유명사 등은 정확히 인용하세요\n"
            
            "### 2. 내용의 풍부성\n"
            "- 참고 자료의 배경 지식을 적극 활용하세요\n"
            "- 뉴스 기사만으로는 알 수 없는 맥락이나 설명을 추가하세요\n"
            "- 전문 용어나 개념에 대한 쉬운 설명을 포함하세요\n"
            
            "### 3. 내용의 완전성\n"
            "- 뉴스의 핵심 내용을 빠뜨리지 마세요\n"
            "- 참고 자료에서 관련된 중요한 정보를 보충하세요\n"
            "- Why, How, What의 관점에서 깊이 있게 다루세요\n\n"
            
            "## 대화 구성 요구사항\n"
            "- 총 3개의 QnA 세트로 구성\n"
            "- 각 대사는 캐릭터답게 자연스럽고 간결하게\n"
            "- 단순 반복이 아닌 새로운 관점이나 추가 정보 포함\n"
            "- 참고 자료의 정보를 자연스럽게 녹여내기\n\n"
